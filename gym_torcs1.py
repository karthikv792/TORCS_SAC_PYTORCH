...
class TorcsEnv:
    ...
    def step(self, u):
       #print("Step")
        ...
        # prev_action = self.agent_to_torcs(u_prev)
        ...
        if self.throttle is False:
            print("KARTHIK: SHOULD NOT BE HERE! ")
            sys.exit()
        ...
        # print("DEGREEE", deg)
        if abs(deg)<90:
            r2 = np.exp(-k2*abs(deg))
        elif deg>=90:
            r2 = -np.exp(-k2*(180-deg))
        else:
            r2 = -np.exp(-k2*(180+deg))
        deg2 = -deg
        ...
        # print("SLIP", slip)
        if abs(slip)<90:
            r3 = np.exp(-k2*abs(slip))
        elif slip>=90:
            r3 = -np.exp(-k2*(180-slip))
        else:
            r3 = -np.exp(-k2*(180+slip))
        r1 = np.clip(r1,0,1)
        r2 = np.clip(r2,-1,1)
        ...
        v = math.sqrt(sp*sp + spy*spy) * 0.2777778
        partial_rewards = {"degree": deg, "trackPos":trackPos, "Reward1": r1, "Reward2": r2, "Reward3": r3, "slip":slip}

        partial_reward = v*(20*r1+20*r2+10*r3)
        print("Partial Rewards",partial_reward)
        progress =partial_reward# + sp*np.cos(obs['angle']) - np.abs(sp*np.sin(obs['angle'])) - sp * np.abs(obs['trackPos'])
        # progress =sp*np.cos(obs['angle']) - np.abs(sp*np.sin(obs['angle'])) - sp * np.abs(obs['trackPos'])
        if v<6 and progress>0:
            progress=progress/2
        reward = progress
...
#---------------------------------------------------
        ...
        #         reward = -2000 # KAUSHIK ADDED THIS
        #         episode_terminate = True
        #         client.R.d['meta'] = True
# #---------------------------------------------------
#
